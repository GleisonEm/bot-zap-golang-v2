...
func (service serviceApp) UploadInWhatsapp(ctx context.Context, dataWaImage []byte) (whatsmeow.UploadResponse, error) {
	return service.WaCli.Upload(ctx, dataWaImage, whatsmeow.MediaImage)
}

func (service serviceApp) SendInWhatsapp(ctx context.Context, contextLog string, message *waE2E.Message, fromChat string) {
	dataWaRecipient, _ := whatsapp.ValidateJidWithLogin(service.WaCli, fromChat)

	send, errorSend := service.WaCli.SendMessage(ctx, dataWaRecipient, message)
	fmt.Println(contextLog, send, errorSend)
}

func (service serviceApp) MakeBuildReaction(ctx context.Context, sender string, stanzaID string, emojiReaction string) *waE2E.Message {
	dataWaRecipientSender, _ := whatsapp.ValidateJidWithLogin(service.WaCli, sender)

	return service.WaCli.BuildReaction(dataWaRecipientSender, types.JID{
		User:   sender, //Agus
		Server: types.DefaultUserServer,
	}, stanzaID, emojiReaction)
}
